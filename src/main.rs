...
#[derive(Component)] struct Block { id: u32 }

#[derive(Component)] struct BlockIndicatorUI;

#[derive(Component)] struct FpsText;

#[derive(Component)] struct CoordsText;


fn load_blocks_from_assets() -> BlockRegistry {
        ...
        for entry in entries.flatten() {
            ...
            if path.is_file() && path.extension().and_then(|s| s.to_str()) == Some("png") {
                ...
                if let Some(stem) = path.file_stem().and_then(|s| s.to_str()) {
                    ...
                    block_names.push(stem.to_string());
    ...
    if let Some(pos) = block_names.iter().position(|x| x == "grid") {

        let grid_name = block_names.remove(pos);

        block_names.insert(0, grid_name);

    } else {

        block_names.insert(0, "grid".to_string());
    ...
    println!("Blocs chargés depuis /assets : {:?}", block_names);


    BlockRegistry { names: block_names }
...
fn main() {
    ...
    let registry = load_blocks_from_assets();
    ...
    App::new()
        ...
        .insert_resource(CurrentBlock { id: 0 })
...
fn setup(
    ...
    for (id, name) in registry.names.iter().enumerate() {
        ...
        let texture_handle = asset_server.load(format!("{}.png", name));
        ...
        block_materials.handles.insert(id as u32, material);

    }


    let box_mesh = meshes.add(Cuboid::new(1.00, 1.00, 1.00));
    ...
    let block_name = registry.names.get(current_block.id as usize).cloned().unwrap_or_else(|| "grid".to_string());

    commands.spawn((

        TextBundle::from_section(

            format!("Selected block : {}", block_name),
...
fn change_block(
    ...
    let total = registry.names.len() as u32;
    ...
    for ev in scroll_evr.read() {
        ...
        if ev.y < 0.0 {
            ...
            current_block.id = (current_block.id + 1) % total;

        } else if ev.y > 0.0 {

            if current_block.id == 0 {

                current_block.id = total - 1;
...
fn update_ui(
    ...
    if current_block.is_changed() {
        ...
        let name = registry.names.get(current_block.id as usize).cloned().unwrap_or_else(|| "Inconnu".to_string());

        for mut text in &mut query {

            text.sections[0].value = format!("Selected block : {}", name);
...
fn raycast_voxels(origin: Vec3, dir: Vec3, max_distance: f32, grid: &WorldGrid) -> Option<(IVec3, IVec3)> {
    ...
    let sample_pos = origin + Vec3::splat(0.5);

    let mut voxel = sample_pos.floor().as_ivec3();
    ...
    let mut t_max = Vec3::new(
        ...
        if dir.x != 0.0 { ((voxel.x as f32 + if step.x > 0 { 1.0 } else { 0.0 }) - sample_pos.x) / dir.x } else { f32::INFINITY },

        if dir.y != 0.0 { ((voxel.y as f32 + if step.y > 0 { 1.0 } else { 0.0 }) - sample_pos.y) / dir.y } else { f32::INFINITY },

        if dir.z != 0.0 { ((voxel.z as f32 + if step.z > 0 { 1.0 } else { 0.0 }) - sample_pos.z) / dir.z } else { f32::INFINITY },
